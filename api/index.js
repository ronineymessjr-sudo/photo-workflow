...
function hashPassword(password) {
    return crypto.createHash('sha256').update(password + '_photoatelier_salt').digest('hex');
}
...
function genToken(userId) {
    const secret = process.env.JWT_SECRET || 'photoatelier-default-secret';
...
function verifyToken(authHeader) {
    ...
    try {
        ...
        const expectedSig = crypto.createHmac('sha256', secret).update(decoded.p).digest('hex');
...
async function handleLogin(body) {
  ...
  const newUser = await sbQuery('users', 'POST', { email, password_hash: hashPassword(password) });
...
async function handleDeleteSchedule(uid, id) {
    // 验证所有权
    const existing = await sbQuery(`schedules?id=eq.${id}&user_id=eq.${uid}&select=id`);
...
async function handleDeleteMessage(uid, id) {
    const existing = await sbQuery(`messages?id=eq.${id}&user_id=eq.${uid}&select=id`);
