...
async function readJson(request) {
  ...
  if (contentLength > 2e4) throw new Error("Payload too large");
  return request.json();
}
...
function isDeployCheck(payload) {
  return payload.build === "deploy-check" && payload.sessionId === "system-check";
}
__name(isDeployCheck, "isDeployCheck");

// api/public/events.js
async function onRequestPost({ request, env }) {
  ...
  `).bind(
    ...
    JSON.stringify(payload.metadata || {}).slice(0, 2e3)
  ).run();
...
// ../.wrangler/tmp/pages-XMdvgw/functionsRoutes-0.32300524688286825.mjs
var routes = [
  {
